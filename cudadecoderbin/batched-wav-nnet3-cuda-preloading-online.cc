...
int main(int argc, char *argv[]) {
  try {
    ...
    KALDI_LOG<<"Done preloading model";
    // starting timer here so we
    // can measure throughput
    // without allocation
    // overheads
    // using kaldi timer, which starts counting in the constructoir
    while(true){
	    ifstream ifile;
	    //ifile.open("scp/wav.scp");
	    KALDI_LOG<<"waiting for file";
	    while(true){
		    ifile.open("scp/wav.scp");
		    if(ifile){
			    break;
	    ...
	    //cout << "Enter wav.scp path ";
	    //cin >> wav_rspecifier;
	    wav_rspecifier = "scp:scp/wav.scp";
    Timer timer;
    CompactLatticeWriter clat_writer(clat_wspecifier);
	...
	KALDI_LOG<<"Decoding with cuda ppeline";
